...
const detailTips={
  ...
  'Status Rates':'Shows how resistant or susceptible this enemy is to each status. Resistant means the enemy is harder to affect with that status; Susceptible means the enemy is easier to affect. For example, 5% Resistant means a 5% reduction in the chance of being affected, while 10% Susceptible means a 10% increase.',
  'Accuracy / Evasion':'Hit Rate is the chance this enemy\'s attack will hit. Evasion is the chance this enemy avoids an incoming attack. For example, 93% Hit Rate means a 93% chance to hit, while +3% Evasion means a 3% chance to evade.',
...
function dropTooltip(d){
  ...
  if(d.type==='weapons'||d.type==='armors'){
    ...
    const vals=paramSummaryText(item);
    if(vals)details.push(vals);
    details.push('Click to view full details.');
  }else{
...
function dropLink(d){
  ...
  return `<a class="drop-item-link" href="#${d.type}/${encodeURIComponent(item.id)}" tabindex="0" data-tooltip-drop="1">${icon(item.iconIndex)}<span class="drop-item-name"><span class="drop-name-text">${esc(item.name)}</span>${tier}</span><span class="tooltip-popup">${esc(tip)}</span></a>`;
}
