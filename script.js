...
function enemyDetail(e){
  const cat=enemyCategory(e), zone=noteTag(e.note,'Tattle Category'), p=e.params||[], img=enemyImage(e), skills=enemySkillEntries(e), drops=enemyDropEntries(e), extraDrops=enemyExtraDrops(e), conditionalDrops=enemyConditionalDrops(e), growth=enemyGrowth(e), traits=enemyTraits(e);
  const {prev,next}=detailNav('enemies',e);
  const base=enemyLevelValues(e,enemyLevelBounds(e).min), statNames=['HP','MP','ATK','DEF','M.ATK','M.DEF','AGI','LUK'], statCaps=[100,30,10,10,10,10,10,10];
  const stats=statNames.map((name,i)=>{
    const value=Number(base.stats[i]||0), width=Math.min(100,Math.max(0,(value/statCaps[i])*100)), tip=detailTips[name]||'';
    return `<div class="stat-row" tabindex="0" data-stat-index="${i}" data-stat-cap="${statCaps[i]}" data-base-value="${value}" data-stat="${esc(name)}"><span class="stat-name">${tip?tipSpan(name,tip,'tooltip-term'):name}</span><div class="bar-track"><div class="bar-fill" data-width="${width.toFixed(1)}%"></div><div class="bar-growth-fill" data-growth-width="0%" data-growth-left="${width.toFixed(1)}%"></div></div><span class="stat-value" data-stat-value>${value}</span></div>`
  }).join('');
...
function bindEnemyLevelPreview(){
  const shell=document.querySelector('.enemy-detail-shell'), select=document.querySelector('.enemy-level-select');
  if(!shell||!select)return;
  const enemy=dataById('enemies',shell.dataset.enemyId);
  if(!enemy)return;
  const bounds=enemyLevelBounds(enemy);
  const update=(level)=>{
    const lv=Math.max(bounds.min,Math.min(bounds.max,Number(level)||bounds.min)), values=enemyLevelValues(enemy,lv);
    shell.classList.toggle('level-preview-active',lv!==bounds.min);
    shell.dataset.previewLevel=String(lv);
    document.querySelectorAll('.stat-row[data-stat-index]').forEach(row=>{
      const i=Number(row.dataset.statIndex), cap=Number(row.dataset.statCap)||100, baseValue=Number(row.dataset.baseValue)||0, value=Number(values.stats[i]||0);
      const baseWidth=Math.min(100,Math.max(0,(baseValue/cap)*100)), totalWidth=Math.min(100,Math.max(0,(value/cap)*100)), growthWidth=Math.max(0,totalWidth-baseWidth);
      const fill=row.querySelector('.bar-fill'), growth=row.querySelector('.bar-growth-fill'), valueEl=row.querySelector('[data-stat-value]');
      if(fill)fill.style.width=`${baseWidth.toFixed(1)}%`;
      if(growth){growth.style.left=`${baseWidth.toFixed(1)}%`;growth.style.width=`${growthWidth.toFixed(1)}%`}
      if(valueEl)valueEl.textContent=String(value);
      row.dataset.previewValue=String(value);
    });
    const rewards={xp:values.exp,gold:values.gold};
    Object.entries(rewards).forEach(([key,val])=>{const box=document.querySelector(`[data-reward="${key}"] [data-reward-value]`);if(box)box.textContent=String(val)});
    document.querySelectorAll('.level-preview-current').forEach(x=>x.textContent=`Level ${lv}`);
    if(select)select.value=String(lv);
  };
