...
function enemyIsBossAdd(e){return String(e?.name||'').trim().toLowerCase()==='tiny turaf slime'}
function enemyIsTrueBoss(e){return enemyCategory(e)==='boss'&&!enemyIsBossAdd(e)}
const ascensionBonuses=[null,{0:1,6:1,7:3},{0:2,1:5},{0:2,3:1,6:1}];
function enemyAscensionValues(e,level,ascension){const base=enemyLevelValues(e,level),a=Math.max(0,Math.min(5,Number(ascension)||0)),stats=base.stats.slice();if(a>=1)for(let lv=1;lv<=Math.min(3,a);lv++)for(const [idx,val] of Object.entries(ascensionBonuses[lv]||{}))stats[Number(idx)]+=Number(val);if(a>=5&&enemyIsTrueBoss(e))for(let i=0;i<stats.length;i++)stats[i]=Math.round(stats[i]*1.5);return {level,ascension:a,stats,exp:base.exp,gold:base.gold,growth:base.growth}}
function enemyPreviewControls(e){const b=enemyLevelBounds(e),levelOptions=[];for(let lv=b.min;lv<=b.max;lv++)levelOptions.push(`<option value="${lv}">Level ${lv}</option>`);const ascOptions=['None','Ascension 1','Ascension 2','Ascension 3','Ascension 4','Ascension 5'].map((x,i)=>`<option value="${i}">${x}</option>`).join('');return `<div class="enemy-preview-controls"><div class="enemy-level-preview"><div class="enemy-level-preview-copy"><span class="enemy-level-preview-label">LEVEL PREVIEW</span><span>Select a level to see calculated stats and rewards. Level growth follows the current enemy-level rules.</span></div><select class="enemy-level-select" data-enemy-level="${esc(e.id)}" aria-label="Enemy level preview">${levelOptions.join('')}</select></div><div class="enemy-level-preview ascension-preview"><div class="enemy-level-preview-copy"><span class="enemy-level-preview-label ascension-preview-label">ASCENSION PREVIEW</span><span>Preview cumulative Ascension bonuses. Ascension 4 changes starting level in-game; Ascension 5’s +50% stat bonus applies to true bosses only.</span></div><select class="enemy-level-select ascension-select" data-enemy-ascension="${esc(e.id)}" aria-label="Ascension level preview">${ascOptions}</select></div></div>`}
function enemyLevelBounds(e){const raw=String(e?.note||'');const staticMatch=raw.match(/<(?:STATIC LEVEL|SET LEVEL):\s*(\d+)>/i);if(staticMatch){const n=Number(staticMatch[1]);return {min:n,max:n,static:true}}const a=raw.match(/<(?:MINIMUM LEVEL|MIN LEVEL):\s*(\d+)>/i);const b=raw.match(/<(?:MAXIMUM LEVEL|MAX LEVEL):\s*(\d+)>/i);return {min:a?Number(a[1]):1,max:b?Number(b[1]):1,static:false}}
function levelRange(e){const r=enemyLevelBounds(e);return `${esc(r.min)}-${esc(r.max)}`}
function enemyJP(e){const m=String(e?.note||'').match(/<JP:\s*([+-]?\d+(?:\.\d+)?)>/i);return m?Number(m[1]):0}
const enemyLevelDefaults={rates:[0.15,0.10,0.05,0.05,0.05,0.05,0.05,0.05,0.15,0.15],flats:[50,10,2.5,2.5,2.5,2.5,2.5,2.5,10,10]};
const enemyLevelKeys=['maxhp','maxmp','atk','def','mat','mdf','agi','luk','exp','gold'];
function enemyLevelGrowth(e,key,index){const raw=String(e?.note||'');const reRate=new RegExp('<'+key+'\\s+Rate:\\s*([+-]?[\\d.]+)([%％])?\\s+per\\s+level>','i');const reFlat=new RegExp('<'+key+'\\s+Flat:\\s*([+-]?[\\d.]+)\\s+per\\s+level>','i');const rm=raw.match(reRate),fm=raw.match(reFlat);let rate=enemyLevelDefaults.rates[index],flat=enemyLevelDefaults.flats[index];if(rm){rate=Number(rm[1]);if(rm[2])rate*=0.01}if(fm)flat=Number(fm[1]);return {rate,flat,rateExplicit:!!rm,flatExplicit:!!fm}}
function enemyLevelValues(e,level){const p=e.params||[],baseValues=[0,1,2,3,4,5,6,7].map(i=>Number(p[i]||0));baseValues.push(Number(e.exp??0),Number(e.gold??0));const ignored=/<IGNORE LEVEL BONUS(?:ES)?>/i.test(String(e.note||''));if(ignored||level<=1)return {level:1,stats:baseValues.slice(0,8),exp:Math.floor(baseValues[8]),gold:Math.floor(baseValues[9]),growth:enemyLevelKeys.map((k,i)=>enemyLevelGrowth(e,k,i)),ignored};const values=baseValues.map((base,i)=>{const g=enemyLevelGrowth(e,enemyLevelKeys[i],i);const raw=base*(1+(level-1)*g.rate)+(g.flat*(level-1));return i>=8?Math.floor(raw):Math.round(raw)});return {level,stats:values.slice(0,8),exp:values[8],gold:values[9],growth:enemyLevelKeys.map((k,i)=>enemyLevelGrowth(e,k,i)),ignored}}
function enemyLevelPreviewControl(e){const bounds=enemyLevelBounds(e);if(bounds.min>=bounds.max)return '';const options=[];for(let lv=bounds.min;lv<=bounds.max;lv++)options.push(`<option value=\"${lv}\">Level ${lv}</option>`);return `<div class=\"enemy-level-preview\"><div class=\"enemy-level-preview-copy\"><span class=\"enemy-level-preview-label\">LEVEL PREVIEW</span><span>Select a level to see calculated stats and rewards. Growth follows the current enemy-level rules.</span></div><select class=\"enemy-level-select\" data-enemy-level=\"${esc(e.id)}\" aria-label=\"Enemy level preview\">${options.join('')}</select></div>`}
...
function skillTargets(){const map={1:'One Enemy',2:'All Enemies',7:'One Ally',11:'Self'};return [...new Set(allPublicSkills().map(s=>Number(s.scope)).filter(Number.isFinite))].sort((a,b)=>a-b).map(n=>[String(n),map[n]||'Other'])}
function skillActionCommands(){const labels={'circle':'Circle','wheel':'Wheel','mash':'Mash','clock':'Clock','arrows':'Arrows'};return [...new Set(allPublicSkills().map(s=>noteValue(s.note,'Timed Attack').toLowerCase()).filter(Boolean))].filter(k=>labels[k]).sort((a,b)=>labels[a].localeCompare(labels[b])).map(k=>[k,labels[k]])}
...
function listPage(type,title,desc,filters){return shell(type,`<div class="page-head fade"><div><div class="page-kicker">Field Codex</div><h1 class="page-title">${title}</h1><p class="page-desc">${desc}</p></div><input class="search" id="search" placeholder="Search ${title.toLowerCase()}..." /></div>${filters?`<div class="filters" id="filters">${filters.map((f,i)=>`<button class="filter ${i===0?'active':''} ${f[2]?'filter-disabled':''}" data-filter="${esc(f[0])}" ${f[2]?'disabled data-tooltip="No recorded entries yet."':''}>${esc(f[1])}</button>`).join('')}</div>`:''}<div class="list-tools"><span class="count" id="count"></span><div class="list-tool-controls">${filterControls(type)}<button class="clear-filters" id="clearFilters" type="button" data-tooltip="Reset filters, search, and sorting to the default view.">CLEAR FILTERS</button>${sortControls(type)}</div></div><div class="grid fade" id="cards"></div>`)}
...
function renderList(type){
  ...
  if(type==='skills'){
    ...
    const typeFilter=$('#skillType')?.value||'all',elementFilter=$('#skillElement')?.value||'all',effectFilter=$('#skillEffect')?.value||'all',costFilter=$('#skillCost')?.value||'all',cdFilter=$('#skillCooldown')?.value||'all',targetFilter=$('#skillTarget')?.value||'all',commandFilter=$('#skillCommand')?.value||'all';
    ...
    if(targetFilter!=='all')arr=arr.filter(x=>String(Number(x.scope))===targetFilter);
    if(commandFilter!=='all')arr=arr.filter(x=>{const command=noteValue(x.note,'Timed Attack').toLowerCase();return commandFilter==='none'?!command:command===commandFilter});
  }
...
function enemyConditionalDrops(e){
  const raw=String(e.note||'');
  const rows=[];
  const blockRe=/<Conditional\s+([^>]+)>([\s\S]*?)<\/Conditional\s+[^>]+>/gi;
  for(const m of raw.matchAll(blockRe)){
    const name=cleanText(m[1]);
    const lines=m[2].split(/\r?\n/);
    for(const line of lines){
      const hit=cleanText(line).match(/^Variable\s+(\d+)\s*(>=|>|<=|<|=)\s*(\d+)\s*:\s*\+?(\d+)%$/i);
      if(!hit)continue;
      let condition=`Variable ${hit[1]} ${hit[2]} ${hit[3]}`;
      if(String(hit[1])==='60'){
        const threshold=Number(hit[3]);
        if(hit[2]==='>=' && threshold===5) condition='Ascension Level is 4 or greater';
        else if(hit[2]==='>=' && threshold===4) condition='Ascension Level is 3 or greater';
        else if(hit[2]==='>' && threshold===4) condition='Ascension Level is greater than 4';
        else if(hit[2]==='>' && threshold===3) condition='Ascension Level is greater than 3';
        else condition=`Ascension Level ${hit[2]} ${Math.max(0,threshold-1)}`;
      }
      rows.push({name,condition,bonus:hit[4]});
    }
  }
  return rows;
}
function enemyGrowth(e){
  const labels=[['maxhp','HP'],['maxmp','MP'],['atk','ATK'],['def','DEF'],['mat','M.ATK'],['mdf','M.DEF'],['agi','AGI'],['luk','LUK'],['exp','XP'],['gold','Gold']];
  const fmt=n=>Number.isInteger(n)?String(n):String(Number(n.toFixed(2)));
  return labels.map(([key,label],i)=>{const g=enemyLevelGrowth(e,key,i);return [label,`${fmt(g.rate*100)}% + ${fmt(g.flat)} / level`]});
}
...
function panelInfo(primary,action){return `<span class="panel-info tooltip-term" tabindex="0"><span class="tooltip-label">👁︎</span><span class="tooltip-popup"><span class="tooltip-primary">${esc(primary)}</span><span class="tooltip-action">${esc(action)}</span></span></span>`}
...
function enemyDetail(e){
  const cat=enemyCategory(e),zone=noteTag(e.note,'Tattle Category'),img=enemyImage(e),skills=enemySkillEntries(e),drops=enemyDropEntries(e),extraDrops=enemyExtraDrops(e),conditionalDrops=enemyConditionalDrops(e),growth=enemyGrowth(e),traits=enemyTraits(e),bounds=enemyLevelBounds(e),base=enemyLevelValues(e,enemyLevelBounds(e).min),statNames=['HP','MP','ATK','DEF','M.ATK','M.DEF','AGI','LUK'],statCaps=[100,30,10,10,10,10,10,10];
  const stats=statNames.map((name,i)=>{const value=Number(base.stats[i]||0),width=Math.min(100,Math.max(0,(value/statCaps[i])*100)),tip=detailTips[name]||'';return `<div class="stat-row" tabindex="0" data-stat-index="${i}" data-stat-cap="${statCaps[i]}" data-base-value="${value}" data-stat="${esc(name)}"><span class="stat-name">${tip?tipSpan(name,tip,'tooltip-term'):name}</span><div class="bar-track"><div class="bar-fill" data-width="${width.toFixed(1)}%"></div><div class="bar-growth-fill" data-growth-width="0%" data-growth-left="${width.toFixed(1)}%"></div><div class="bar-ascension-fill" data-asc-width="0%" data-asc-left="${width.toFixed(1)}%"></div></div><span class="stat-value" data-stat-value>${value}</span></div>`}).join('');
  ...
  const study=enemyStudy(e),stickyImg=img?`<img src="${img}" alt="">`:icon(e.iconIndex),levelControl=enemyPreviewControls(e),bossAddTag=enemyIsBossAdd(e)?`<span class="tag boss-add-tag">Boss Add</span>`:'';
  const facts=`<div class="fact-grid"><div class="fact" data-reward="xp"><strong data-reward-value>${base.exp}</strong><small>XP</small></div><div class="fact" data-reward="gold"><strong data-reward-value>${base.gold}</strong><small>Gold</small></div><div class="fact"><strong>${enemyJP(e)}</strong><small>Abyss Essence (AE)</small></div><div class="fact"><strong>${esc(levelRange(e))}</strong><small>Level Range</small></div></div>`;
  const previewHint=bounds.min<bounds.max?`<span class="panel-hint">Level ${bounds.min} shown by default · calculated preview available</span>`:`<span class="panel-hint">Base values · fixed scale</span>`;
  return `<div class="enemy-detail-shell" data-enemy-id="${esc(e.id)}" data-level-min="${bounds.min}" data-level-max="${bounds.max}">${sideNavHtml('enemies',e)}<div class="detail-page fade"><a class="back" href="#enemies">← Back to Enemy Bestiary</a><div class="detail-top enemy-detail-top"><div class="detail-art enemy-detail-art">${img?`<img src="${img}" alt="${esc(e.name)}">`:icon(e.iconIndex)}</div><div class="enemy-header-copy"><div class="enemy-header-meta">${enemyTag(cat)}${bossAddTag}${zone?`<span class="tag enemy-zone-tag">${esc(cleanText(zone))}</span>`:''}</div><h1>${esc(e.name)}</h1></div></div><div class="detail-layout"><section class="panel enemy-stats-panel"><div class="panel-title enemy-stats-title"><span>Stats</span>${previewHint}</div>${levelControl}${stats}${facts}</section><section class="panel"><div class="panel-title">Traits</div>${traitHtml}</section><section class="panel"><div class="panel-title">Drops</div>${dropHtml}${extraHtml}${conditionalHtml}</section><section class="panel"><div class="panel-title">Growth Rates</div>${growthHtml}</section><section class="panel enemy-expandable study-panel" data-expand="study"><div class="panel-title"><span class="study-title-label">Study Description</span> ${panelInfo('When you use the Study ability on this enemy, this is the description recorded in its Study Log.','Click to open an expanded view of the Study Description.')}</div>${study?`<div class="expandable-scroll"><p class="detail-description">${esc(study)}</p></div>`:'<div class="expandable-scroll"><p class="detail-description">No study description recorded.</p></div>'}</section><section class="panel enemy-expandable skills-panel" data-expand="skills"><div class="panel-title">Skills <span class="panel-count">${skills.length}</span> ${panelInfo('This panel lists the skills recorded for this enemy.','Click to open an expanded view of this enemy’s recorded skills.')}</div><div class="skill-list">${skillHtml}</div></section></div></div><div class="enemy-sticky-id" aria-hidden="true"><div class="enemy-sticky-art">${stickyImg}</div><strong>${esc(e.name)}</strong></div><div class="detail-expand-modal" id="detailExpandModal" aria-hidden="true"><div class="detail-expand-backdrop"></div><div class="detail-expand-dialog" role="dialog" aria-modal="true"><button class="detail-expand-close" type="button" aria-label="Close expanded view">×</button><div class="detail-expand-title"></div><div class="detail-expand-body"></div></div></div></div>`;
}
...
function helpPage(){const groups=[['Getting Started', [['What is the encyclopedia?','This site is a player-facing reference for Infinite Echoes in The Infinite Abyss. Use it to browse enemies, equipment, items, skills, status effects, characters, and other game information.'],['How do I browse entries?','Choose a section from the navigation, then select an entry to open its detailed page. On detail pages, Previous and Next stay available at the sides so you can move through entries without returning to the list.'],['How do I return to the top?','After you scroll down, the ↑ TOP button appears in the lower-right corner. Select it to return to the top of the current page.']]],['Finding Information', [['How does Search work?','Use the search field in the top-right to search across the encyclopedia. Search is useful when you already know the name of an enemy, item, weapon, armor, skill, status effect, or character.'],['What do the filters do?','List pages can include filters for things such as enemy categories, equipment tiers, item groups, or skill categories. Disabled filters indicate that there are no recorded entries in that category yet.'],['What do the rarity tiers mean?','Rarity applies to equipment. Tier 0 is Starter, followed by Common, Uncommon, Rare, Epic, Legendary, and Mythic. The colors shown in the rarity scale are used throughout equipment entries.']]],['Enemy Pages', [['What is the Study Description?','The Study Description is the text recorded in an enemy’s Study Log when the Study ability is used on that enemy. Select the Study Description panel or its information icon to open a larger view.'],['What do the enemy stats mean?','Enemy pages show their core battle statistics, XP, Gold, Abyss Essence, level range, traits, drops, and growth rates. Stat bars are visual references; the number shown beside each stat is the authoritative value.'],['What is Level Preview?','Enemies with a range of possible levels can be previewed at each level shown by the selector. The encyclopedia recalculates their stats, XP, and Gold using the same level-growth rules used by the game. The gold overlay shows the added growth above the base value.'],['What is Ascension Preview?','Use the Ascension Preview alongside Level Preview to see cumulative Ascension bonuses. Ascension 4 is shown as an in-game starting-level modifier, while Ascension 5’s +50% all-stat bonus applies only to true bosses.'],['Why are some drops or entries interactive?','Dotted-underlined names can provide a preview when hovered. Select an interactive entry when you want to open its full encyclopedia page. The eye icon on expandable panels indicates that the panel can be opened for a larger view.']]],['Reading the Data', [['Why might information change?','The encyclopedia is a living reference. Enemy values, equipment stats, descriptions, drops, and other data may change as the game is balanced and updated. Check Version History for recorded encyclopedia revisions.'],['What does Resistant or Susceptible mean?','These terms describe how strongly an enemy is affected by an element or status effect. Resistant means the enemy is affected less than normal, while Susceptible means the enemy is affected more than normal.'],['What does Hit Rate or Evasion mean?','Hit Rate is the chance an attack successfully hits. Evasion is a bonus to the chance of avoiding an attack.']]],['Accessibility', [['What does Spoilers Off do?','When spoilers are off, entries marked as spoilers are visually obscured. Hovering a spoiler entry explains that it contains spoiler information, and enabling spoilers reveals the entry normally.'],['Can I change the text size?','Use the A−, A, and A+ controls in the accessibility dock to adjust the encyclopedia text size. Your selected text size is saved for later visits.']]]];return shell('help',`<div class="page-head fade"><div><div class="page-kicker">Player Guide</div><h1 class="page-title">Help & FAQ</h1><p class="page-desc">A quick guide to navigating the encyclopedia, understanding its information, and getting the most out of its interactive features.</p></div></div><div class="help-grid fade">${groups.map(([title,rows])=>`<section class="panel help-group"><div class="panel-title">${esc(title)}</div><div class="help-list">${rows.map(([q,a])=>`<details class="help-entry"><summary>${esc(q)}<span>+</span></summary><p>${esc(a)}</p></details>`).join('')}</div></section>`).join('')}</div>`)}
function ascensionPage(){const rows=[['1','All enemies gain +1 ATK, +1 AGI, and +3 LUK.'],['2','All enemies gain +2 Max HP and +5 Max MP.'],['3','All enemies gain +2 Max HP, +1 DEF, and +1 AGI.'],['4','Enemies start 1 level higher in-game. This changes their starting level rather than directly adding stats.'],['5','True bosses gain +50% to all stats. Boss Add enemies do not receive this bonus. Ascension 5 can also enable additional Echo Shard drop behavior where applicable.']];return shell('ascension',`<div class="page-head fade"><div><div class="page-kicker">Difficulty System</div><h1 class="page-title">Ascension Levels</h1><p class="page-desc">A reference for the escalating modifiers used by Ascension Levels. Enemy pages can preview the cumulative stat effects alongside level scaling.</p></div></div><div class="ascension-grid fade">${rows.map(([n,d])=>`<section class="panel ascension-card"><div class="ascension-number">${n}</div><div><div class="panel-title">Ascension ${n}</div><p class="detail-description">${esc(d)}</p></div></section>`).join('')}</div><div class="notice ascension-note">Ascension bonuses are cumulative. The enemy Level Preview and Ascension Preview can be used together to see their combined effect on an enemy’s displayed stats. Ascension Level 5’s +50% stat bonus applies only to true bosses, not Boss Add enemies.</div>`)}
function glossaryPage(){const groups=[['Core Terms',[['Abyss Essence','A reward earned from defeating enemies. It is the player-facing name used for the game’s JP-style enemy reward.'],['Enemy Level Preview','A player-facing calculator for enemies with a level range. It shows the calculated stats, XP, and Gold at the selected level.'],['Ascension Level','A difficulty level that applies cumulative modifiers to enemies. Ascension 4 changes their starting level, while Ascension 5 gives true bosses a +50% all-stat bonus.'],['Skill Type','The category a skill belongs to.'],['Item Type','The category an item belongs to.'],['Scope','Determines which target or targets an action can affect.'],['Occasion','Determines when an action can be used. Battle Screen means it can be used during battle, while Menu Screen means it can be used from the menu.'],['Invocation Speed','Changes an action\'s speed relative to the user\'s Agility. Positive values make it happen sooner; negative values make it happen later. A +1 action effectively has 1 more Agility for that action.'],['Success','The base chance for the action to successfully affect its target.'],['Repeats','How many times the action is performed when used.']]],['Damage & Accuracy',[['Hit Type','Determines how an action checks whether it hits. Certain Hit ignores normal accuracy and evasion checks, Physical uses physical accuracy and evasion, and Magical uses magical hit rules.'],['Damage Type','Describes whether an action deals damage, restores HP or MP, or drains HP or MP.'],['Element','The elemental type associated with the action. Normal Attack Element uses the user\'s normal attack element.'],['Variance','The amount of random variation applied to the calculated damage or recovery amount.'],['Critical Hit','Whether the action can produce a critical hit.']]],['Costs & Timing',[['MP Cost','The amount of MP consumed when the action is used.'],['EP Cost','The amount of EP consumed when the action is used.'],['EP Gain','The amount of EP gained when the action is used.'],['Cooldown','How many turns must pass before the action can be used again.'],['Warm-up','How many turns the action takes before it can be used.']]],['Health Conditions',[['Danger','HP is at 25% or lower.'],['Critical HP','HP is at 10% or lower.']]],['Equipment',[['Weapon Type','The weapon category assigned to a weapon.'],['Armor Type','The armor category assigned to armor or an accessory.'],['Unique Only','Only one piece of equipment with this restriction can be equipped at a time.']]]];return shell('glossary',`<div class="page-head fade"><div><div class="page-kicker">Reference Guide</div><h1 class="page-title">Glossary</h1><p class="page-desc">A quick reference for the terminology used throughout the encyclopedia.</p></div></div><div class="glossary-grid fade">${groups.map(([title,rows])=>`<section class="panel glossary-group"><div class="panel-title">${esc(title)}</div><div class="glossary-list">${rows.map(([term,desc])=>`<article class="glossary-entry"><h3>${esc(term)}</h3><p>${esc(desc)}</p></article>`).join('')}</div></section>`).join('')}</div>`)}
...
function bindAccess(){
  ...
  if(topBtn){
    const updateTop=()=>topBtn.classList.toggle('visible',window.scrollY>260);
...
function route(){document.body.classList.remove('modal-open');const raw=(location.hash||'#home').slice(1)||'home';document.body.innerHTML=page(raw);applyPrefs();window.scrollTo({top:0,left:0,behavior:'auto'});nav(raw.split('/')[0]);bindGlobal();bindAccess();bindCardRoutes();bindDetailSideNav();if(!raw.includes('/')&&raw!=='home'){const base=raw;renderList(base);bindCardRoutes();$('#search')?.addEventListener('input',()=>{renderList(base);bindCardRoutes()});$('#sort')?.addEventListener('change',()=>{renderList(base);bindCardRoutes()});document.querySelectorAll('.filter-select').forEach(el=>el.addEventListener('change',()=>{renderList(base);bindCardRoutes()}));document.querySelectorAll('.filter').forEach(b=>b.onclick=()=>{if(b.disabled)return;document.querySelectorAll('.filter').forEach(x=>x.classList.remove('active'));b.classList.add('active');renderList(base);bindCardRoutes()});$('#clearFilters')?.addEventListener('click',()=>resetListControls(base))}if(raw.includes('/')&&raw.split('/')[0]==='enemies'){setTimeout(animateBars,70);setTimeout(bindEnemyLevelPreview,0);setTimeout(bindEnemySticky,0);setTimeout(bindEnemyExpand,0)}}
function bindDetailSideNav(){}
function bindEnemyLevelPreview(){const shell=document.querySelector('.enemy-detail-shell'),levelSelect=document.querySelector('.enemy-level-select:not(.ascension-select)'),ascSelect=document.querySelector('.ascension-select');if(!shell)return;const enemy=dataById('enemies',shell.dataset.enemyId);if(!enemy)return;const bounds=enemyLevelBounds(enemy);const update=()=>{const lv=Math.max(bounds.min,Math.min(bounds.max,Number(levelSelect?.value)||bounds.min)),asc=Math.max(0,Math.min(5,Number(ascSelect?.value)||0)),base=enemyLevelValues(enemy,bounds.min),levelVals=enemyLevelValues(enemy,lv),values=enemyAscensionValues(enemy,lv,asc),active=lv!==bounds.min||asc>0;shell.classList.toggle('level-preview-active',active);shell.dataset.previewLevel=String(lv);shell.dataset.previewAscension=String(asc);document.querySelectorAll('.stat-row[data-stat-index]').forEach(row=>{const i=Number(row.dataset.statIndex),cap=Number(row.dataset.statCap)||100,baseValue=Number(base.stats[i]||0),levelValue=Number(levelVals.stats[i]||0),finalValue=Number(values.stats[i]||0),baseWidth=Math.min(100,Math.max(0,(baseValue/cap)*100)),levelWidth=Math.min(100,Math.max(0,(levelValue/cap)*100)),totalWidth=Math.min(100,Math.max(0,(finalValue/cap)*100)),fill=row.querySelector('.bar-fill'),growth=row.querySelector('.bar-growth-fill'),ascFill=row.querySelector('.bar-ascension-fill'),valueEl=row.querySelector('[data-stat-value]');if(fill)fill.style.width=`${baseWidth.toFixed(1)}%`;if(growth){growth.style.left=`${baseWidth.toFixed(1)}%`;growth.style.width=`${Math.max(0,levelWidth-baseWidth).toFixed(1)}%`}if(ascFill){ascFill.style.left=`${levelWidth.toFixed(1)}%`;ascFill.style.width=`${Math.max(0,totalWidth-levelWidth).toFixed(1)}%`}if(valueEl)valueEl.textContent=String(finalValue)});Object.entries({xp:levelVals.exp,gold:levelVals.gold}).forEach(([key,val])=>{const box=document.querySelector(`[data-reward="${key}"] [data-reward-value]`);if(box)box.textContent=String(val)});};if(levelSelect)levelSelect.addEventListener('change',update);if(ascSelect)ascSelect.addEventListener('change',update);update()}
